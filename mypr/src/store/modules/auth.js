  ...
  export default {
    ...
    actions: {
      async saveLoginUserData({ commit }, loginResult) {
        ...
        console.log(loginResult); //--------- user data -------
        const user = loginResult?.user; // об'єкт користувача
        commit("setUser", user); //user.displayName - ім'я акаунту //user.photoURL - аватар акаунту //user.email - електронна адреса акаунту //--------- user access token ------- // This gives you a Google Access Token. You can use it to access the Google API.
        ...
        return new Promise((resolve, reject) => {
          ...
          if (credential) {
            ...
            signInWithCredential(auth, credential)
              ...
              .catch((error) => {
                console.log(error);
                commit("setError", error); // Handle Errors here. //var errorCode = error.code //var errorMessage = error.message // The email of the user's account used. //var email = error.email
                reject(false);
              });
          } else resolve(false);
        });
